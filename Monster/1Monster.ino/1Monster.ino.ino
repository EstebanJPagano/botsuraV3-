...
char databt = 0;      // variable para almacenar caracter recibido
#define POWERBT 49
#define GNDRBT 40
...
int speedMIN = 130;      // variable para almacenar valor de velocid
int speedOff = 0 ;

// ----------- definir led
int PinLED1 = 22; //blanco adelante
int PinLED2 = 23; //blanco adelante
int PinLED3 = 24; //traseras
int PinLED4 = 25; //Sierna
int PinLED5 = 26; //amarillo atras
int PinLED6 = 27; //
...
void setup() {
  ...
  pinMode(POWERBT, OUTPUT);
  digitalWrite(POWERBT,HIGH);
  pinMode(GNDRBT,OUTPUT);//define a digital pin as output
  digitalWrite(GNDRBT, LOW);// set the above pin as LOW so it acts as Ground
...
void loop() {
  ...
  if (miBT.available()) {    // si hay informacion disponible desde modulo
    ...
    Serial.write(databt);
      ...
      if (databt == '0'){ Stop(); goLucesOff();}
      else if (databt == '1'){ Forward(); goLucesAdelante(); }
      else if (databt == '3'){ Reverse(); goLucesReversa(); }
      else if (databt == '4'){ Left(); goLucesIzquierda(); }
      else if (databt == '2'){ Right(); goLucesDerecha();}
      
      else if (databt == 'x'){ danceDance();}
      
      else if(databt == 'c') { SetSpeedCarMax(); }
      else if(databt == 'b') { SetSpeedCarMed();  }
      else if(databt == 'a') { SetSpeedCarMin(); }
      
      else if(databt == 'R') { goLuces(); }
      else if(databt == 'N') { goLucesOff(); }
      else if(databt == 'G') { led_blink(); }
      else if(databt == 'B') { goLucesColor(); } 
      else if(databt == 'Y') { goLucesOffColor(); }
...
void goLuces(){ 
   ...
   digitalWrite(PinLED3, HIGH);  // sirena
   digitalWrite(PinLED4, HIGH); // luces reversa
   digitalWrite(PinLED5, HIGH);  
   digitalWrite(PinLED6, HIGH);  // atras naranja
}
